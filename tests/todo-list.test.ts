...
describe("adding a todo with a due date", () => {
  ...
  it("keeps its due date when the Date passed in is later mutated", () => {
    const list = new TodoList();
    const dueDate = new Date("2026-01-01");
  ...
  it("keeps its due date when the Date handed back out is later mutated", () => {
    const list = new TodoList();
    const dueDate = new Date("2026-01-01");
...
describe("listing todos in due-date order", () => {
  it("answers dated todos soonest first, whatever order they were added in", () => {
    ...
    const middle = list.add("middle", new Date("2026-02-01"));
  ...
  it("keeps todos sharing a due date in the order they were added", () => {
    const list = new TodoList();
    const dueDate = new Date("2026-01-01");
    const first = list.add("first", new Date(dueDate));
    const second = list.add("second", new Date(dueDate));

    expect(list.list("all", "dueDate").map((todo) => todo.id)).toEqual([first.id, second.id]);
  });

  it("keeps undated todos in the order they were added", () => {
    const list = new TodoList();
  ...
  it("answers a filtered listing in due-date order", () => {
    const list = new TodoList();
    const later = list.add("later", new Date("2026-03-01"));
    const soonest = list.add("soonest", new Date("2026-01-01"));
...
describe("listing overdue todos", () => {
  it("answers a dated, open todo due before the clock's now", () => {
    ...
    const overdue = list.add("buy milk", new Date("2026-01-01"));

    expect(list.overdue().map((todo) => todo.id)).toEqual([overdue.id]);
  });

  it("does not answer a dated, open todo due after the clock's now", () => {
    const now = new Date("2026-01-15");
    const list = new TodoList({ now: () => now });
  ...
  it("answers overdue todos in the order they were added", () => {
    ...
    list.add("undated");
    const third = list.add("third", new Date("2026-01-01"));
  ...
  it("measures against the real clock when constructed without one", () => {
    const list = new TodoList();
    const pastDue = list.add("buy milk", new Date("2000-01-01"));
    list.add("buy bread", new Date("3000-01-01"));

    expect(list.overdue().map((todo) => todo.id)).toEqual([pastDue.id]);
  });
});
