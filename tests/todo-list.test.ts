...
describe("searching todos", () => {
  ...
  it("answers matches in the order they were added", () => {
    ...
    expect(list.search("milk").map((todo) => todo.title)).toEqual(["buy oat milk", "buy milk"]);
  });
  ...
  it("answers a fresh listing that a later add does not reach", () => {
    ...
    const found = list.search("milk");
    list.add("buy more milk");

    expect(found).toHaveLength(1);
  });

  it("matches text found anywhere in the title", () => {
    const list = new TodoList();
    list.add("buy organic milk");

    expect(list.search("organic").map((todo) => todo.title)).toEqual(["buy organic milk"]);
  });
