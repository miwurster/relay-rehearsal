...
describe("adding a todo after removing one", () => {
  it("adds beside what remains, without losing or overwriting a todo", () => {
    const list = new TodoList();
  ...
  it("never mints an id a still-held todo is already using, across many interleaved removes and adds", () => {
    const list = new TodoList();
    const a = list.add("a");
    const b = list.add("b");
    list.remove(a.id);
    ...
    expect(list.list().map((todo) => todo.title)).toEqual(["f"]);
    expect(f.id).not.toBe(a.id);
    expect(f.id).not.toBe(b.id);
    expect(f.id).not.toBe(c.id);
