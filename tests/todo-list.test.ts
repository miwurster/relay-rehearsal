...
describe("renaming a todo", () => {
  ...
  it("keeps the due date it was added with", () => {
    const list = new TodoList();
    const dueDate = new Date("2026-08-10");
...
describe("completing and reopening a todo", () => {
  ...
  it("keeps the due date across completing and reopening", () => {
    const list = new TodoList();
    const dueDate = new Date("2026-08-10");
    const added = list.add("buy milk", dueDate);
  ...
  it("does not gain or change a due date underneath a caller who mutates their own Date", () => {
    const list = new TodoList();
    const dueDate = new Date("2026-08-10");
    const added = list.add("buy milk", dueDate);
    ...
    expect(added.dueDate).toEqual(new Date("2026-08-10"));
  });

  it("does not gain or change a due date underneath a caller who mutates the returned todo", () => {
    const list = new TodoList();
...
describe("listing todos in due-date order", () => {
  ...
  it("answers every undated todo after every dated one", () => {
    ...
    expect(list.list("all", "due-date").map((todo) => todo.title)).toEqual(["dated", "someday"]);
  });

  it("keeps the order todos sharing a due date were added in, and the undated ones among themselves", () => {
    const list = new TodoList();
    const dueDate = new Date("2026-08-01");
    list.add("first dated", dueDate);
    list.add("first undated");
    list.add("second dated", dueDate);
  ...
  it("orders the open ones by due date when asked for open", () => {
    const list = new TodoList();
    const completed = list.add("completed", new Date("2026-07-01"));
    list.add("later", new Date("2026-09-01"));
    list.add("sooner", new Date("2026-08-01"));
  ...
  it("orders the completed ones by due date when asked for completed", () => {
    const list = new TodoList();
  ...
  it("keeps insertion order when asked for no order, even with due dates that would otherwise reorder it", () => {
    const list = new TodoList();
    list.add("later", new Date("2026-09-01"));
    list.add("sooner", new Date("2026-08-01"));
...
describe("listing overdue todos", () => {
  ...
  it("does not hold a todo due exactly at the clock's now", () => {
    const now = new Date("2026-08-01T12:00:00Z");
    const list = new TodoList({ now: () => now });
    list.add("renew passport", new Date(now.getTime()));

    expect(list.overdue()).toEqual([]);
  });

  it("does not hold a completed todo, however long past its due date", () => {
    const now = new Date("2026-08-01T12:00:00Z");
    const list = new TodoList({ now: () => now });
    const passport = list.add("renew passport", new Date("2000-01-01T00:00:00Z"));
  ...
  it("still answers when constructed without a clock", () => {
    const list = new TodoList();
