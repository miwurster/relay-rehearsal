...
describe("searching todos", () => {
  ...
  it("matches text anywhere in the title, not only at its start", () => {
    const list = new TodoList();
    list.add("go to the milk farm");

    expect(list.search("milk").map((todo) => todo.title)).toEqual(["go to the milk farm"]);
  });
