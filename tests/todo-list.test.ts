...
describe("removing a todo", () => {
  ...
  it("does not make a later add overwrite a todo that is still in the list", () => {
    const list = new TodoList();
  ...
  it("empties the list when every todo is removed, and adds again cleanly", () => {
    const list = new TodoList();
    const first = list.add("first");
    const second = list.add("second");
    list.remove(first.id);
  ...
  it("keeps every todo in place through many interleaved removes and adds", () => {
    const list = new TodoList();
    const a = list.add("a");
    const b = list.add("b");
    list.remove(a.id);
