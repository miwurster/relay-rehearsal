...
describe("searching todos", () => {
  ...
  it("answers nothing when the text is empty once trimmed", () => {
    const list = new TodoList();
    list.add("buy milk");
  ...
  it("answers an empty listing rather than an error when nothing matches", () => {
    const list = new TodoList();
    list.add("buy milk");
  ...
  it("answers matches in the order they were added", () => {
    ...
    expect(list.search("milk").map((todo) => todo.title)).toEqual(["buy oat milk", "buy soy milk"]);
  });
  ...
  it("searches within the open todos", () => {
    const list = new TodoList();
    const milk = list.add("buy milk");
    list.add("buy soy milk");
    list.complete(milk.id);

    expect(list.search("milk", "open").map((todo) => todo.title)).toEqual(["buy soy milk"]);
  });

  it("searches within the completed todos", () => {
    const list = new TodoList();
    const milk = list.add("buy milk");
    list.add("buy soy milk");
    list.complete(milk.id);
