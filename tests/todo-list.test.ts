...
describe("searching todos", () => {
  ...
  it("matches anywhere in the title, not only at its start", () => {
    ...
    expect(list.search("milk").map((todo) => todo.title)).toEqual(["buy oat milk"]);
  });
  ...
  it("answers in insertion order", () => {
    const list = new TodoList();
    list.add("milk first");
    list.add("milk second");

    expect(list.search("milk").map((todo) => todo.title)).toEqual(["milk first", "milk second"]);
  });
