...
describe("searching todos", () => {
  ...
  it("ignores case in both the title and the text", () => {
    const list = new TodoList();
    list.add("Buy Milk");
  ...
  it("trims whitespace from the text before matching", () => {
    const list = new TodoList();
    list.add("buy milk");
  ...
  it("composes with a filter, searching only among the todos it asks for", () => {
    const list = new TodoList();
    const milk = list.add("buy milk");
    list.add("buy oat milk");
    list.complete(milk.id);
