 ...
 * for another: `list` returns todos in the order they were added unless
 * asked for due-date order instead, and adding never reorders what is
 * already there.
 */
export class TodoList {
  ...
  /** The todos the filter asks for, in the given order (insertion order by default). */
  list(filter: TodoFilter = "all", order: TodoOrder = "insertion"): Todo[] {
    const todos = [...this.todos.values()].filter((todo) => matches(todo, filter)).map(expose);
    return order === "dueDate" ? todos.sort(byDueDate) : todos;
  }
...
/** A todo safe to hand to a caller: its due date is a copy, so mutating it cannot reach the stored todo. */
...
/** Soonest due date first; every undated todo after every dated one; ties keep insertion order. */
