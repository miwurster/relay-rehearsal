...
export class TodoList {
   ...
   * in the past is accepted.
   */
  add(title: string, dueDate?: Date): Todo {
    const accepted = requireTitle(title);
    const acceptedDueDate = dueDate === undefined ? undefined : requireValidDueDate(dueDate);
...
function requireValidDueDate(dueDate: Date): Date {
  if (isNaN(dueDate.getTime())) {
    throw new InvalidDueDateError("A todo's due date must be a usable point in time.");
  }
  return dueDate;
}
