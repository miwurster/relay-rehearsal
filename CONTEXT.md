...
**Due date**: the point in time a **todo** is meant to be done by, set only when the todo is added.
A due date already in the past is accepted, not an error.
...
**Due-date order**: a **listing**'s order by soonest **due date** first, with every **undated** todo after every dated one.
Todos sharing a due date, and undated todos among themselves, keep **insertion order** relative to each other.
_Avoid_: sort order, chronological order, date order.
