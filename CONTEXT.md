...
A due date in the past is accepted — a todo somebody is already late on is ordinary, not an error. A due date that is not a usable point in time is refused with `InvalidDueDateError`, before it joins the list. Once a todo has a due date, renaming, completing and reopening it leave the due date as it was.
