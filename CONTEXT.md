...
**Due date**: the point in time a **todo** is meant to be done by, given optionally when it is added and kept unchanged after that.
...
_Avoid_: no date, blank date, null due date.
...
**Due-date order**: the order a **listing** comes back in when asked for it — soonest **due date** first, with every **undated** todo after every dated one.
Todos sharing a due date, and the undated todos among themselves, keep **insertion order**.
...
**Clock**: the **todo list**'s source of now, supplied to it when it is constructed.
