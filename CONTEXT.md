...
**Due date**: the point in time a **todo** is meant to be done by, accepted when the todo is added and kept unchanged by renaming, completing and reopening it.
A due date in the past is accepted.
A value that is not a usable point in time is refused with `InvalidDueDateError`.
_Avoid_: deadline, target date.

**Undated**: a **todo** added without a **due date**, distinguishable from one carrying any date.
_Avoid_: no date, null date.
...
**Due-date order**: an order a **listing** can be asked for, the soonest **due date** first and every **undated** todo after every dated one.
...
**Overdue**: a **todo** that is dated, **open**, and due strictly before the **clock**'s now.
...
**Clock**: what a **todo list** reads its now from, supplied to it when it is constructed.
A list constructed without one reads the real clock.
One constructed with one is measured against that instead.
