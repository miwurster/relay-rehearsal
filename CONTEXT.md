...
**Search**: which **todo**s a **listing** holds whose **title** contains some text, case-folded and trimmed on both sides.
Text that is empty once trimmed matches nothing, and a match anywhere in the title counts.
Search composes with **filter**: a **todo list** can be searched within the open todos, the completed ones, or all of them.
_Avoid_: query, text filter.
