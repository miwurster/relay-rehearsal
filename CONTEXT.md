...
A todo due exactly at now is not yet overdue, a completed todo is never overdue however late it once was, and an **undated** todo is never overdue.
_Avoid_: late, expired.

**Clock**: the source of "now" a **todo list** measures **overdue** against, supplied optionally when the list is constructed.
A list constructed without one reads the real clock; a list constructed with one is measured against that instead, so tests can pin "now" without sleeping or drifting with the calendar.
