...
**Todo**: one thing somebody means to do, carrying an **id**, a **title**, its completion and an optional **due date**.
...
**Due date**: the point in time a **todo** is meant to be done by, given when it is added and optional.
A due date that is not a usable point in time is refused with `InvalidDueDateError`, before anything joins the list.
...
**Due-date order**: a **listing** ordered by soonest **due date** first, askable of any **filter**.
...
_Avoid_: sorted order, date order.
...
A **due date** equal to now is not yet overdue, a **completed** todo never is, and neither is an **undated** todo.
The overdue answer comes back in **insertion order**, like any other **listing**.
_Avoid_: late, past due.

**Clock**: the **todo list**'s source of now, supplied by the caller when the list is constructed.
A **todo list** built without one reads the real clock, so only tests that pin **overdue**-ness need to supply their own.
